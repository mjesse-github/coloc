...
def trim(bf1, bf2, p1=1e-4, p2=1e-4, overlap_min=0.5, silent=True):
    ...
    if all(drop):
        ...
        return pd.DataFrame({
            'nsnps': [len(isnps)],
            'hit1': [bf1.columns[pp1.argmax(axis=1)[todo['i']]]],
            'hit2': [bf2.columns[pp2.argmax(axis=1)[todo['j']]]],
            'PP.H0.abf': [np.nan], 'PP.H1.abf': [np.nan], 'PP.H2.abf': [np.nan], 'PP.H3.abf': [np.nan], 'PP.H4.abf': [np.nan],
            'idx1': todo['i'], 'idx2': todo['j']
        })
...
if __name__ == "__main__":
    for root, dirs, files in os.walk("p"):
        for directory in dirs:
            ...
            for pair in tqdm(combination, desc="Running files: "): 
                input1 = pd.read_parquet(f"{dir_path}/{files[i]}")
                metadata1 = input1.iloc[:, :5].copy()  
                mat1 = input1.iloc[:, 5:].copy()

                input2 = pd.read_parquet(f"{dir_path}/{files[j]}")
                metadata2 = input2.iloc[:, :5].copy()  
                mat2 = input2.iloc[:, 5:].copy()
